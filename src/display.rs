...
pub fn setup_background(
    ...
    for i in (-(BOX_HEIGHT as i32) / 2)..(BOX_HEIGHT as i32 / 2) {
        for j in (-(BOX_WIDTH as i32) / 2)..(BOX_WIDTH as i32 / 2) {
            if board.entities.contains_key(&Position{x:j, y:i}) {
                spawn_tile_with_texture(&mut commands, &mut meshes, &mut materials, wall_image.clone(), Position {x:j, y:i}, 1.);
            }
            else {
                spawn_tile_with_texture(&mut commands, &mut meshes, &mut materials, tile_image.clone(), Position {x:j, y:i}, 0.);
            }
        }
    }
}
