...
mod level_select;
mod main_menu;
use crate::{state::DisplayState, exit::handle_esc};
...
impl Plugin for MenusPlugin {
    fn build(&self, app: &mut App) {
        app.add_system_set(
            ...
            SystemSet::on_resume(DisplayState::MainMenu).with_system(setup_main_menu),
        )
        .add_system_set(SystemSet::on_update(DisplayState::MainMenu).with_system(handle_menu_click).with_system(handle_esc))
        ...
        .add_system_set(
            SystemSet::on_update(DisplayState::LevelSelect).with_system(handle_level_click).with_system(handle_esc)
        )
        .add_system_set(
