...
pub fn handle_ice(
    ...
    for position in movement_data.moved_positions.iter() {
        ...
        let mut object_position = position.next_position(direction);
        if let Floor::Warp(map) = board.get_floor_type(object_position) {
            object_position = board.get_warp_position(map, board.get_current_map());
        }
        ...
        match object {
            ...
            GameObject::Box => {
                    ...
                    .contains(&position.next_position(direction))
                {
                    ...
                    if let Floor::Warp(map) = board.get_floor_type(next_object_position) {
                        next_object_position = board.get_warp_position(map, board.get_current_map());
                    }
